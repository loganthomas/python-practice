...
requires no further fuel. So, the total fuel required for a module of mass
...
def calc_fuel(data):
    ...
    # Floor division by 3 and subtract 2
    ind_mod_fuel = (data // 3) - 2

    total_fuel = ind_mod_fuel.sum()
...
def calc_single_fuel(int_):
    return int(int_ // 3 - 2)


def calc_recursive_fuel(int_):
    total_sum = 0
    fuel = calc_single_fuel(int_)

    while fuel > 0:
        total_sum += fuel
        fuel = calc_single_fuel(fuel)
