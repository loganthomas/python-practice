...
by looking at the first integer (called position 0). Here, you will find an opcode - either 1, 2, or
...
(The halt instruction would increase the instruction pointer by 1, but it halts the program
instead.)
...
def perform_opr(data_list, index):
    ...
    if opr == 1:
        ...
        raise ValueError(f"Did not recognize opr={opr} as '1', '2', or '99'")

    # Update ans in data_list
    data_list[ans_index] = ans

    return data_list


def update_list(data_list):

    # start at first element
    index = 0

    while data_list[index] != 99:
        data_list = perform_opr(data_list, index)
        index += 4
...
def part_1_answer(data_path=data_path):
    ...
    data_list = update_list(data_list)
...
def update_list_using_tuple(data_list):
    ...
    while data_list[instruction_pointer] != 99:
        ...
        data_list = perform_tuple_opr(current_tuple, data_list)

        instruction_pointer += len(current_tuple)

    return data_list


def part_2_answer(data_path=data_path, expected=19690720):
    # Read in puzzle as text
    text = data_path.read_text().strip()
